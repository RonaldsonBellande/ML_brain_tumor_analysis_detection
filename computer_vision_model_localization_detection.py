...
class computer_vision_localization_detection(object):
    ...
    def decode_netout(self, predicted_classes_network, anchors):
        ...
        nb_box = 3
        predicted_classes_network = predicted_classes_network.reshape((grid_h, grid_w, nb_box, -1))
    ...
    def localization(self):
        ...
        for image in os.listdir(self.image_path):
            ...
            image, image_w, image_h = self.load_image_pixels(image_name)
            ...
            for i in range(len(predicted_classes)):
                boxes += self.decode_netout(predicted_classes[i][0], anchors[i], class_threshold)
            ...
            v_boxes, v_labels, v_scores = self.get_boxes(boxes, class_threshold)

            # draw what we found
            self.draw_boxes(image_name, v_boxes, v_labels, v_scores)
